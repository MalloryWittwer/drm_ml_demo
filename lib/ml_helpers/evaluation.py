...
def visu_preds_and_targets(preds, minieulers, rx, ry):
    ...
    xmap_pred, ymap_pred, zmap_pred = visualize(preds, rx, ry, reshape=False)
    ...
    xmap, ymap, zmap = visualize(minieulers, rx, ry, reshape=False)
    xmap = xmap.numpy().reshape((rx,ry,3))
    ymap = ymap.numpy().reshape((rx,ry,3))
    zmap = zmap.numpy().reshape((rx,ry,3))
...
def evaluate_and_disptlot(model, testing_dataset, yte, title='test'):
    ...
    maX, maY, maZ = in_and_out_of_plane(preds, yte)
...
def voronoi_IPF_plot(eulers, z_values, direction='z'):
    ...
    def get_region_values(vor, xyc, zees):
        ...
        for ind, points in neighbors_dico.items():
            regmedian[ind] = np.median(points)
    ...
    # region_values, region_medians = get_region_values(vor, xy_coords, z_values)
